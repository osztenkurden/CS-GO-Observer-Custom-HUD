...
function updatePage(data) {
    ...
    if (phase) {
        ...
        //winner
        if (phase.phase == "over" || (phase.phase == "freezetime" && phase.phase_ends_in > 7)) {
            if (round.win_team) {
                var winnerTeam = teams.left.side == round.win_team.toLowerCase()
                    ? teams.left
                    : teams.right;
                $('#win_announcer').html(winnerTeam.name + " wins the round.");
                if (winnerTeam.logo) {
                ...
                if (round.win_team == "CT") {
                    $('#win_announcer_border').css(
                        "background",
                        "linear-gradient(to left, rgba(87,136,168,0), rgba(87,136,168,1))"
                    );
                }
        ...
        if (phase.phase == "bomb" || phase.phase == "defuse") {
            ...
            if (phase.phase == "defuse") {
                ...
                $("#defuse_bar").css("width", 220 * (parseFloat(phase.phase_ends_in) / longd) + "px");
